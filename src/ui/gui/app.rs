...
impl Autom8App {
    ...
    fn render_session_card_inner(
        ...
        // Check if session has pause requested
        let is_paused = session.metadata.pause_requested;
        ...
        // State text - show "Stopped" for interrupted, "(Not responding)" if stuck, otherwise machine state
        let state_text = if is_interrupted {
            "Stopped".to_string()
        } else if appears_stuck {
...
mod tests {
    ...
    fn test_us004_removal_summary_returned_from_backend() {
        ...
        // Use a unique name with timestamp to avoid conflicts from previous runs
        ...
        let result = remove_project_direct(&unique_name);
        assert!(
            result.is_ok(),
